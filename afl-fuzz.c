...
//linux have sched_setaffinity
#ifdef __linux__
# define HAVE_AFFINITY 1
#endif
//export some variables when building as a library
#ifdef AFL_LIB
# define EXP_ST
#else
...
//Interesting values, in config.h
...
EXP_ST void setup_signal_handlers(void){
  ...
  sigemptyset(&sa.sa_mask);//initial
  ...
  sigaction(SIGPIPE, &sa, NULL);//pipe broken
}
...
//Trim and possibly create a banner
...
//classify execution counts, must be fast
...
//Mark deterministic checks as done for a particular queue entry
//use file to avoid repeating deterministic fuzzing when resuming aborted scans
...
//Create hard links for input test cases in the output directory, choosing good names and pivoting accordingly
...
//we don't want to keep auto-scaling the timeout over and over
...
//exists and isn't a shell script a valid ELF header and for evidence of AFL instrumentation
...
//bump into a new path, call this to see if the path appears more "favorable" than any of the existing ones
...
//Create symlinks if possible to make it easier to examine the files
...
//processing the input directory
...
//dry run of all test cases to confirm that the app is working as expected. for initial only once
...
//Mark / unmark as redundant (edge-only), may be useful for post-processing datasets
...
//sequentially grabs winners for previously-unseen bytes (temp_v) and marks them as favored
...
//save or queue the input test case for further analysis. Returns 1 if entry is saved, 0 otherwise
...
//Read bitmap from file. This is for the -B option
...
int main(int argc, char** argv){
  ...
  struct timeval tv;
  struct timezone tz;
  SAYF(cCYA "afl-fuzz " cBRI VERSION cRST " by scut\n");
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QV")) > 0){
    switch(opt){
      ...
      //load bitmap
      case 'B':
        /*It is useful if you find
           an interesting test case during a normal fuzzing process, and want
           to mutate it without rediscovering any of the test cases already
           found during an earlier run.

           To use this mode, you need to point -B to the fuzz_bitmap produced
           by an earlier run for the exact same binary
        */
  ...
  bind_to_free_cpu();
#endif
  check_crash_handling();///proc/sys/kernel/core_pattern
  check_cpu_governor();//check if performance

  setup_post();//custom lib
  setup_shm();
  init_count_class16();
